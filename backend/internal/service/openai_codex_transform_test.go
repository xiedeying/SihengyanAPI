...
func TestApplyCodexOAuthTransform_BoundsLongCallIDsAndPreservesPairing(t *testing.T) {
	...
	reqBody := map[string]any{
		"model": "gpt-5.2",
		"input": []any{
			map[string]any{"type": "function_call", "call_id": "call_" + suffix, "name": "shell"},
			map[string]any{"type": "function_call_output", "call_id": "fc_" + suffix, "output": "done"},
		},
	}
	...
	require.True(t, ok)
	outputCallID, ok := output["call_id"].(string)
	require.True(t, ok)
...
func TestStripOpenAIImageGenerationTools_RemovesToolAndToolChoice(t *testing.T) {
	reqBody := map[string]any{
		"tools": []any{
			map[string]any{"type": "web_search"},
			map[string]any{"type": "image_generation", "output_format": "png"},
...
func TestStripCodexSparkImageGenerationTools_RemovesLastTool(t *testing.T) {
	reqBody := map[string]any{
		"model":       "gpt-5.3-codex-spark",
...
func TestFilterCodexInput_PreservesReasoningButStripsID(t *testing.T) {
	build := func() []any {
		return []any{
			map[string]any{"type": "message", "id": "msg_0", "role": "user", "content": "hi"},
			map[string]any{
