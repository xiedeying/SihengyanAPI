...
func TestReconcileOpenAIResponsesImageResultSizes_PreservesMetadataWhenDetectionFails(t *testing.T) {
	results := []openAIResponsesImageResult{{Result: "not-image-data", Size: "1024x1024"}}
	firstMeta := openAIResponsesImageResult{Size: "1024x1024"}

	reconcileOpenAIResponsesImageResultSizes(results, &firstMeta)

	require.Equal(t, "1024x1024", results[0].Size)
	require.Equal(t, "1024x1024", firstMeta.Size)
}
...
func TestOpenAIGatewayServiceForwardImages_OAuthStreamingUsesDecodedOutputDimensions(t *testing.T) {
	run := runOpenAIOAuthImageActualSizeTest(t, true, false)
...
func TestOpenAIGatewayServiceForwardImages_OAuthStreamingEOFPendingUsesDecodedOutputDimensions(t *testing.T) {
	run := runOpenAIOAuthImageActualSizeTest(t, true, true)
