...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte) (shouldDisable bool) {
	...
	// Anthropic 官方 5h / 7d 窗口耗尽是账号级硬限制，必须优先于自定义 429 临时摘除规则。
	// 否则宽泛的 "rate limit" 规则可能把多小时/多天冷却窗口缩短成本地短暂停。
	...
	case 401:
		...
		if account.Type == AccountTypeOAuth && account.Platform != PlatformAntigravity {
			...
			// 原实现用请求开始时的 account 快照整列覆盖 credentials JSONB（见
...
// apply429FallbackRateLimit 在无法解析上游重置时间时应用可配置的秒级默认回避。
// 并发/RPM 类 429 是瞬时的，长冷却会把健康账号误伤到号池外；这里用短回避让账号
// 在当前在途请求消退后快速恢复，同时通过 settings 表允许运行时调整或关闭。
func (s *RateLimitService) apply429FallbackRateLimit(ctx context.Context, account *Account, reason string) {
	...
	resetAt := time.Now().Add(cooldown)
	slog.Warn("rate_limit_429_fallback_used", "account_id", account.ID, "platform", account.Platform, "reason", reason, "cooldown", cooldown.String())
	if err := s.accountRepo.SetRateLimited(ctx, account.ID, resetAt); err != nil {
		slog.Warn("rate_limit_set_failed", "account_id", account.ID, "error", err)
	}
}

// get429FallbackCooldown 返回429默认回避时长及是否启用。优先读 settings 表，
// 读不到或读取失败时回退到默认 5 秒。
...
func calculateOpenAI429ResetTime(headers http.Header) *time.Time {
	...
	// 未达到 100% 时，reset-after 只代表窗口信息，不能证明账号配额耗尽。
	// 这类瞬时 429 必须回到可配置的兜底路径，避免未耗尽账号被长时间排除。
	return nil
}
...
// 测试路径直接验证数据库中的最新凭据；恢复 OAuth 账号时需要同步清理 token 缓存，
// 避免后续转发继续命中过期 access_token 后又把账号打回 error。
