...
func (s *AccountTestService) FetchUpstreamSupportedModels(ctx context.Context, account *Account) ([]string, error) {
	...
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		if account.IsGrok() && resp.StatusCode == http.StatusPaymentRequired {
			if _, stateErr := setGrokPaymentRequiredErrorIfMatch(ctx, s.accountRepo, account); stateErr != nil {
				return nil, newUpstreamModelSyncUpstreamError(
					"Grok model list returned HTTP 402, but the account could not be marked as error",
					stateErr,
				)
			}
		}
...
func grokUpstreamModelEntryID(entry upstreamModelEntry) string {
	...
	if len(entry.Meta) > 0 {
		var metadata upstreamModelEntryMetadata
		if err := json.Unmarshal(entry.Meta, &metadata); err == nil {
			candidates = append(candidates,
				metadata.Model,
				metadata.ModelID,
				metadata.ModelIDSnake,
				metadata.ID,
				metadata.Name,
			)
		}
