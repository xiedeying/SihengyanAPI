...
// 授权后回写账号会被广场守卫拦下（账号仍挂在房间、房间不是已暂停、需要 force 等）。
// api client 的拦截器 reject 的是扁平对象、没有 response 字段，旧写法
// error.response.data.detail 恒为 undefined，把后端给出的具体原因整个吞掉了。
...
const handleGenerateUrl = async () => {
  ...
  if (isOpenAILike.value) {
    await openaiOAuth.generateAuthUrl(props.account.proxy_id, { accountLevel: props.account.account_level })
