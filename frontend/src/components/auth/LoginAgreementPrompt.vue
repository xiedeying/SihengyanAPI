<template>
      ...
      <div class="min-w-0 flex-1">
        <p class="text-[13px] leading-5 text-gray-600 dark:text-dark-300">
          ...
          <template v-for="(doc, index) in documents" :key="doc.id || doc.title">
            ...
            <span v-if="index < documents.length - 1">{{ t('auth.agreement.consentSeparator') }}</span>
          </template>
        </p>
        <p
          v-if="hasError"
  ...
  >
    <div class="flex items-start gap-3">
      ...
      />
      <div class="min-w-0 flex-1">
        <p class="font-medium">{{ t('auth.agreement.requiredBeforeAction', { action: actionName }) }}</p>
        <p
          class="mt-1"
          :class="hasError ? 'text-red-700 dark:text-red-200/80' : 'text-primary-700 dark:text-primary-200/80'"
        >
          {{ hasError ? errorMessage : t('auth.agreement.reviewBeforeSubmit') }}
        </p>
      </div>
      <button
        type="button"
  ...
  <ModalShell
    :show="dialogVisible"
    :title="t('auth.agreement.updatedTitle')"
    :close-on-escape="false"
    overlay-class="overflow-y-auto bg-gray-950/60 p-4 backdrop-blur-md"
        ...
        <div class="flex items-start gap-4">
          ...
          <div class="min-w-0 flex-1">
            ...
            <p class="mt-2 text-sm leading-6 text-gray-600 dark:text-dark-300">
              {{ t('auth.agreement.updatedDesc', { date: updatedAt || t('auth.agreement.updatedRecent') }) }}
            </p>
          </div>
        </div>
      </div>
...
const actionName = computed(() => props.actionName.trim() || t('auth.agreement.defaultAction'))
